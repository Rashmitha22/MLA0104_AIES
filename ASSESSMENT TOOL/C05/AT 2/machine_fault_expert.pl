...
all_present(_, []).
all_present(M, [S|Ss]) :- symptom(M,S), all_present(M,Ss).
...
verify_fault(M, Fault) :- probable_fault(M, Fault).
...
scenario(1) :- clear_case, add(m1,abnormal_vibration), add(m1,radial_vibration), add(m1,reduced_speed).
scenario(2) :- clear_case, add(m2,abnormal_vibration), add(m2,axial_vibration), add(m2,excessive_temperature).
scenario(3) :- clear_case, add(m3,abnormal_vibration), add(m3,unusual_noise), add(m3,excessive_temperature).
scenario(4) :- clear_case, add(m4,pressure_change), add(m4,reduced_speed), add(m4,unstable_operation).
scenario(5) :- clear_case, add(m5,excessive_temperature), add(m5,reduced_speed), add(m5,high_current).
scenario(6) :- clear_case, add(m6,abnormal_vibration), add(m6,unusual_noise), add(m6,excessive_temperature), add(m6,bearing_heat).
